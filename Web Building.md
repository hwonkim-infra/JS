...
배경색, 패딩 등으로 탐색 모음과 탐색 모음 링크의 스타일을 지정.
배경색, 패딩 등으로 드롭다운 버튼의 스타일을 지정.
.dropdown 클래스는 .dropdown-content의 컨테이너입니다. 이것은 <a> 요소가 아니라 <div> 요소이므로 링크 옆에 유지되도록 플로팅.

.dropdown-content 클래스에는 실제 드롭다운 메뉴가 있습니다. 기본적으로 숨겨져 있으며 마우스를 가져가면 표시됩니다(아래 참조). min-width는 160px로 설정되어 있습니다. 자유롭게 변경하세요.

테두리를 사용하는 대신 box-shadow 속성을 사용하여 드롭다운 메뉴가 "카드"처럼 보이도록 . 또한 z-index를 사용하여 드롭다운을 다른 요소 앞에 배치합니다.
...
<p>How to create side-by-side images with the CSS float property. On screens that are 600px wide or less, the images will stack on top of each other instead of next to each other.</p>
...
# How TO - Collapse
애니메이션을 축소할 수 있게 하려면 패널 클래스에 max-height: 0, overflow: hidden 및 max-height 속성에 대한 전환을 추가.

그런 다음 JavaScript를 사용하여 다양한 화면 크기에서 패널의 높이에 따라 계산된 최대 높이를 설정하여 콘텐츠를 아래로 내린다.
