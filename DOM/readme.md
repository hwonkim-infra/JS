...
HTML DOM 메서드는 HTML 요소에서 수행할 수 있는 작업.
...
DOM에서 모든 HTML 요소는 객체 로 정의된다 .
프로그래밍 인터페이스는 각 개체의 속성과 메서드
- property: 당신이 (HTML 요소의 내용을 변경하는 등) 가져 오거나 설정할 수 있는 값.
- method: 당신이 (추가 또는 HTML 요소를 삭제 등) 할 수있는 작업.
...
```
input 요소 노드 객체는 prototype에 바인딩된 객체를 상속받음: 즉 모든 prototype property나 method를 상속받아 사용할 수 있다.
...
```
## 요소 노드 취득
HTML 구조나 내용 또는 스타일을 동적으로 조작하기 위해 먼저 요소 노드를 취득해야 함:

id 값은 HTML 에서 유일한 값이어야 함. 공백 문자로 구분되지 않음.    
단 html 문서 내 중복된 id 값을 갖는 요소가 여러 개 존재할 수는 있다. 
```html
<!DOCTYPE html>
<html>
  <body>
